...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post("/auth/login", { ...form, role: "TPO" });
      setUser(data.user);
      toast.success("Welcome, TPO Admin!");
      navigate("/");
    } catch (err) {
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8">
        <div className="text-center mb-8">
          <div className="inline-flex h-14 w-14 items-center justify-center rounded-full bg-blue-100 mb-4">
            <span className="text-2xl font-bold text-blue-700">T</span>
          </div>
          <h1 className="text-2xl font-bold text-[#1E3A5F]">TPO Admin Panel</h1>
          <p className="text-gray-500 text-sm mt-1">NIT Delhi Placement Portal</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="email" required value={form.email}
              ...
              placeholder="tpo@nitdelhi.ac.in"
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? "Logging in…" : "Login as TPO"}
          </button>
        </form>
        <p className="text-center text-xs text-gray-400 mt-5">Restricted access — authorized TPO personnel only.</p>
      </div>
    </div>
  );
};
